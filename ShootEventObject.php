...
/**********Intrinsic Event Properties*************/
    ...
    public function setNumTargetsPerShooter() {
        ...
        $this->numTargetsPerShooter = $c;
        return $this->numTargetsPerShooter;
    }
    
    //Gets the total number of trappers
    public function getTrappers($type) {
        $count = 0;
        foreach ($this->stations as $station) {
            if ($station->getUse() === false) {
                continue;
            }
            foreach ((array)$station->getMachines() as $machine) {
                if ($machine->getManual() === false) {
                    continue;
                } else if ($machine->getTrapper() == $type) {
                    $count++;
                }
            }
        }
    ...
    public function getMachinePreviousEvent($machine,$machines,$events) {
        usort($events,"sortDate");
        $idx = findName($this->getName(),$events,1);
        if ($idx === false) {
            return false;
        } else if ($idx == 0) {
            return 0;
        } else {
            $name = $events[$idx-1]->getName();
            $machines = getAllInArray($machine->getName(),$machines,"name");
            $b = getAllInArray($name,$machines,"event");
            return $b;
        }
...
/*********Display functions**************/
    ...
    $a = array("name" => $this->name,
               ...
               "numStations" => $this->getNumStations(),
               "numTrappersWanted" => $this->getTrappers("Wanted"),
               "numTrappersNeeded" => $this->getTrappers("Needed"));
        return $a;
    }
...
HERE;
        ...
        foreach($machines as $machine) {
            ...
            if (dispTargetColour($mType)) {
                $mColour = "Target colour: " . $machine->getTargetColour() . "<br/>";
            } else {
                $mColour = "";
            }
